...
class BodyPartSlotAttention(nn.Module):
    """Action-slot style attention with body-part priors over spatiotemporal tokens."""
    ...
    def __init__(
        ...
        super().__init__()
        slot_layout = resolve_part_slot_layout(slot_preset, int(num_slots))
        self.slot_layout = slot_layout
        self.slot_names = [str(part["name"]) for part in slot_layout]
        self.slot_preset = str(slot_preset)
        self.num_slots = len(slot_layout)
        self.background_slot = bool(background_slot)
        self.total_slots = self.num_slots + int(self.background_slot)
        self.dim = int(dim)
        self.scale = self.dim ** -0.5
        self.prior_strength = float(prior_strength)

        self.slot_embed = nn.Parameter(torch.randn(1, self.total_slots, self.dim) * 0.02)
        ...
        priors = build_part_priors(
            ...
            background_slot=self.background_slot,
        )
        self.register_buffer("slot_priors", priors, persistent=False)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        q = self.to_q(self.norm_slots(self.slot_embed.expand(batch_size, -1, -1)))
        k = self.to_k(self.norm_inputs(tokens))
        v = self.to_v(tokens)

        logits = torch.einsum("bsd,bnd->bsn", q, k) * self.scale
        prior_logits = self.slot_priors.view(1, self.total_slots, -1).to(device=logits.device, dtype=logits.dtype)
        logits = logits + self.prior_strength * prior_logits

        attn_over_slots = F.softmax(logits, dim=1)
        slot_weights = attn_over_slots / attn_over_slots.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        slots = torch.einsum("bnd,bsn->bsd", v, slot_weights)
        slots = slots + self.ffn(slots)

        slot_maps = slot_weights.view(batch_size, self.total_slots, num_frames, height, width)
        return slots[:, :self.num_slots], slot_maps[:, :self.num_slots]
...
class PhaseAwarePartSlotAggregator(nn.Module):
    """Apply soft phase masks before extracting body-part slots from spatial features."""
    ...
    def __init__(
        ...
        if spatial_resolution is not None:
            self.slot_attention = self._build_slot_attention(spatial_resolution)
    ...
    def _build_slot_attention(self, resolution: Sequence[int]) -> BodyPartSlotAttention:
        return BodyPartSlotAttention(
            num_slots=self.num_part_slots,
    ...
    def _maybe_init_slot_attention(self, spatial_features: torch.Tensor):
        if self.slot_attention is not None:
            return
        _, num_frames, height, width, _ = spatial_features.shape
        resolution = (num_frames, height, width)
        self.slot_attention = self._build_slot_attention(resolution).to(spatial_features.device)
    ...
    def forward(self, spatial_features: torch.Tensor, phase_weights: torch.Tensor):
        ...
        self._maybe_init_slot_attention(spatial_features)
        ...
        part_tokens, part_attn = self.slot_attention(weighted_maps)
